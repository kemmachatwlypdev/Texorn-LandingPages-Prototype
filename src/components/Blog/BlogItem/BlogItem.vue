    ...
    <v-row no-gutters justify="start" v-bind="this.blogsdata[id]">
      <v-col align="start" cols="1">
        ...
        <v-btn
          ...
          dark v-bind="attrs" v-on="on" class="mt-0">
      ...
      <v-col align="start" cols="10">
        <v-row no-gutters>
          <v-col cols="12">
             <img v-bind:src="getImg(blogs[id-1].image)" alt="RPA" width="90%" />
          </v-col>
        </v-row>
      </v-col>
    </v-row>
...
export default {
  ...
  mounted(){
    ...
    axios.get('src\components\Blog\BlogItem\dataBlogs\data.js').then((response) => {
